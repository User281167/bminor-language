    ...
    def ID(self, t):
        if len(t.value) > MAX_ID_LENGTH:
            return self.log_error(
                LexerError.INVALID_ID, t, f"exceeds max length of {MAX_ID_LENGTH}"
            )
        return t
        ...
        if len(content) == 4 and content.startswith("\\x"):
            try:
                ...
                if not (32 <= hex_val <= 126):
                    return self.log_error(
                        LexerError.MALFORMED_CHAR, t, "hex value out of ASCII range"
                    )
            except ValueError:
                return self.log_error(LexerError.MALFORMED_CHAR, t, "invalid hex value")
        elif len(content) == 1 and content[0] == "\\":
            return self.log_error(
                LexerError.MALFORMED_CHAR, t, "incomplete escape sequence"
            )
